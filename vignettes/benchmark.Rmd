...
vignette: >
  %\VignetteIndexEntry{Benchmark Of Distinctiveness}
...
benchmark = function(dataset, times = 20) {
  ...
  bench = microbenchmark(
    table = table_distinctiveness(dataset[[2]], sp_col = "species",
                                  com = "com", abund = NULL,
                                  dist_matrix = dataset[[3]]),
    matrix = distinctiveness(dataset[[1]],
                             dataset[[3]]),
    times = times
  )
...
```{r smallDataSquare}
smallest_square = benchmark(gen_data(10, 10, 0.5), 2)

small_square = benchmark(gen_data(50, 50, 0.5), 2)
```
...
```{r smallDataRect}
small_rect_sp = benchmark(gen_data(10, 100, 0.5), 2)

small_rect_sites = benchmark(gen_data(100, 10, 0.5), 2)
```
...
```{r largeDataSquare, eval=FALSE}
large_square = benchmark(gen_data(1000, 1000, 0.5), 2)
larger_square = benchmark(gen_data(3000, 3000, 0.5), 2)
```


### Rectangular

```{r largeDataRect, eval=FALSE}
large_rect_sites = benchmark(gen_data(3000, 400, 0.5), 2)
large_rect_sp = benchmark(gen_data(400, 3000, 0.5), 2)

larger_rect_sites = benchmark(gen_data(10000, 1000, 0.5), 2)
larger_rect_sp = benchmark(gen_data(1000, 10000, 0.5), 2)
```
