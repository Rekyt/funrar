...
alternative_distinctiveness = function(pres_mat, distance_obj, given_T) {
  ...
  apply(dist_mat, 2, function(given_dist_col) {
    # Find species within range
    kept_species = names(given_dist_col[given_dist_col <= given_T])
    
    di_mat = funrar::distinctiveness(pres_mat[, kept_species, drop = FALSE],
                                     dist_mat)
    funrar::matrix_to_stack(di_mat, "Di", "Site", "Species")
  })
}
```
...
all_T = lapply(seq(0.5, 1.5, length.out = 50),
       function(given_number) alternative_distinctiveness(species_distance,
                                                          given_number))
