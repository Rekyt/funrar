...
In their article, Violle et al. (2016) introduced 4 indices to compute functional rarity indices. `funrar` package enables ecologists to compute these on any dataset.
...
Users can also use a stacked data.frame, containing columns for sites, species and abundances, as an input. The package contains for each metric another species function called `metricname_stack()` function that can take these stacked data.frames.
...
The package is also able to deal with large matrices by using a sparse matrix approach. For more details, see `sparse_matrices` vignette.
...
Using `funrar` distinctiveness values can be computed using function `distinctiveness()`:
...
The syntax for `distinctiveness()` is first the table and then the distance matrix. The function returns a site-species matrix filled with `Di` values, i.e. the species distinctiveness for each species per site combination.
...
Violle C., Thuiller W., Mouquet N., Munoz F., Kraft NJ., Cadotte MW., Livingstone SW., Mouillot D. (2016) Functional rarity: the ecology of outliers. *in revision*
